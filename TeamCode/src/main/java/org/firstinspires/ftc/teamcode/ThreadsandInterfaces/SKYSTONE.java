package org.firstinspires.ftc.teamcode.ThreadsandInterfaces;

public class SKYSTONE {
    public static final double FIELD_WIDTH = 3.5814;
    public static final double ROBOT_WIDTH = 0.4572;
    public static final double DISTANCE_TO_STONES = 1.1938;
    public static final double QUARRY_LENGTH = 1.2319;
    public static final double FOUNDATION_LENGTH = 0.8763;
    public static final double FOUNDATION_WIDTH = 0.4699;
    public static final double WALL_TO_FOUNDATION = 0.1016;
    public static final double START_TO_FOUNDATION = 1.20015;
    public static final double SKYBRIDGE_LENGTH = 1.168;
}
