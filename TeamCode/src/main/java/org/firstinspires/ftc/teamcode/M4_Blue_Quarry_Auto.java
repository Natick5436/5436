...
@Autonomous(name = "M4 Blue Quarry", group = "Autonomous")
@Disabled
public class M4_Blue_Quarry_Auto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException{
        ...
        robot.initialize(hardwareMap, ROBOT_WIDTH, 1.22, -Math.PI/2);
        waitForStart();
        approachStones(0.08, 0.01,0.5);
        robot.setOdometryPosition(DISTANCE_TO_STONES - 0.08, 1.22);
        moveToSkystone(1);
        procureSkystone(0.4);
        robot.goToAbsolutePosition( DISTANCE_TO_STONES - 0.1, 2.0, 1);
    ...
    public void approachStones(double goalDistance, double goalDistanceAcc, double power)throws InterruptedException{
        ...
        sleep(1000);
        robot.turn(0.05, robot.initialAngle);
    ...
    public void procureSkystone(double power) throws InterruptedException{
        robot.forwardMeters(-0.17-ROBOT_WIDTH, power);
        robot.turn(power, 0);
        robot.forwardMeters(0.3, power);
        robot.turn(power,  Math.PI/2);
        robot.turn(0.1,Math.PI/2);
        robot.flyL.setPower(-0.5);
        robot.flyR.setPower(0.5);
        robot.forwardMeters(-0.3, power);
        Thread.sleep(1000);
